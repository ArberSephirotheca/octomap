...
CUDADriverBase::CUDADriverBase() {
  disabled_by_env_ = (get_environ_config("RW_ENABLE_CUDA", 1) == 0);
  if (disabled_by_env_) {
    RW_TRACE("CUDA driver disabled by enviroment variable \"RW_ENABLE_CUDA\".");
  }
}
...
bool CUDADriverBase::load_lib(std::string lib_linux, std::string lib_windows) {
  ...
  if (!loader_->loaded()) {
    RW_WARN("{} lib not found.", lib_name);
    return false;
  } else {
    RW_TRACE("{} loaded!", lib_name);
    return true;
  }
}
...
CUDADriver::CUDADriver() {
  ...
  RW_TRACE("CUDA driver API (v{}.{}) loaded.", version / 1000,
           version % 1000 / 10);
