...
CUDAContext::CUDAContext()
  ...
  RW_TRACE("Using CUDA device [id=0]: {}", name);
  ...
  RW_TRACE("Emitting CUDA code for {}", mcpu_);
}
...
void CUDAContext::launch(void *func,
  ...
  */
  ...
  // from 2000FPS to 1000FPS because of this. RW_ASSERT(grid_dim <=
  // get_current_program().config.saturating_grid_dim); RW_ASSERT(block_dim <=
