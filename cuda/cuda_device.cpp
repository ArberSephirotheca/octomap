...
RedwoodResult CudaDevice::allocate_memory(const AllocParams &params,
...
void CudaDevice::dealloc_memory(DeviceAllocation handle) {
  ...
  if (info.ptr == nullptr) {
    RW_ERROR("the DeviceAllocation is already deallocated");
  }
  RW_ASSERT(!info.is_imported);
...
RedwoodResult CudaDevice::upload_data(DevicePtr *device_ptr,
                                  const void **data,
                                  size_t *size,
...
RedwoodResult CudaDevice::readback_data(
...
RedwoodResult CudaDevice::map(DeviceAllocation alloc, void **mapped_ptr) {
