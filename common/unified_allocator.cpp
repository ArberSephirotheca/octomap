...
void *UnifiedAllocator::allocate(std::size_t size,
  ...
  if (!chunks_.empty() && !exclusive) {
    ...
    for (size_t chunk_id = 0; chunk_id < chunks_.size(); chunk_id++) {
      ...
      RW_TRACE("UM [data={}] allocate() request={} remain={}", (intptr_t)data,
               size, (tail - head));
      head = ret + allocation_size;
      if (head <= tail) {
        // success
        RW_ASSERT(ret % alignment == 0);
  ...
  RW_TRACE("Allocating virtual address space of size {} MB",
