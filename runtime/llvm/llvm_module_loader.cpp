#include "runtime/llvm/llvm_aot_module_loader.h"
#include "runtime/llvm/aot_graph_data.h"
...
std::unique_ptr<aot::CompiledGraph> LlvmAotModule::get_graph(
  ...
  if (it == graphs_.end()) {
    RW_DEBUG("Cannot find graph {}", name);
    return nullptr;
  }
...
void allocate_aot_snode_tree_type(aot::Module *aot_module,
  ...
  RW_ASSERT(llvm_aot_module != nullptr);
  RW_ASSERT(aot_field_impl != nullptr);
