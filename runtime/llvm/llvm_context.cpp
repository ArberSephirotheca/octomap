...
#include "redwood/util/lang_util.h"
#include "redwood/common/task.h"
#include "redwood/util/environ_config.h"
#include "llvm_context.h"
#include "redwood/runtime/program_impls/llvm/llvm_program.h"

#include "redwood/runtime/llvm/llvm_context_pass.h"
...
redwoodLLVMContext::redwoodLLVMContext(const CompileConfig &config, Arch arch)
    : config_(config), arch_(arch) {
  RW_TRACE("Creating redwood llvm context for arch: {}", arch_name(arch));
  main_thread_id_ = std::this_thread::get_id();
  main_thread_data_ = get_this_thread_data();
  llvm::remove_fatal_error_handler();
  llvm::install_fatal_error_handler(
      [](void *user_data, const char *reason, bool gen_crash_diag) {
        RW_ERROR("LLVM Fatal Error: {}", reason);
      },
      nullptr);

  if (arch_is_cpu(arch)) {
#if defined(RW_PLATFORM_OSX) and defined(RW_ARCH_ARM)
  ...
  data_layout_ = redwoodLLVMContext::get_data_layout(arch);
  ...
  RW_TRACE("redwood llvm context created.");
}

redwoodLLVMContext::~redwoodLLVMContext() {
}

llvm::Type *redwoodLLVMContext::get_data_type(DataType dt) {
...
std::string find_existing_command(const std::vector<std::string> &commands) {
  ...
  for (const auto &cmd : commands) {
    RW_WARN("Potential command {}", cmd);
  }
  RW_ERROR("None command found.");
}
...
std::unique_ptr<llvm::Module> redwoodLLVMContext::clone_module_to_context(
    llvm::Module *module,
  ...
  if (!cloned) {
    ...
    RW_ERROR("Bitcode cloned failed.");
  }
  return std::move(cloned.get());
}

std::unique_ptr<llvm::Module>
redwoodLLVMContext::clone_module_to_this_thread_context(llvm::Module *module) {
  RW_TRACE("Cloning struct module");
  RW_ASSERT(module);
...
std::unique_ptr<llvm::Module> LlvmModuleBitcodeLoader::load(
  ...
  RW_ERROR_IF(!ifs, "Bitcode file ({}) not found.", bitcode_path_);
  ...
  if (!runtime) {
    ...
    RW_WARN("Bitcode loading error message:");
    llvm::errs() << error << "\n";
    RW_ERROR("Failed to load bitcode={}", bitcode_path_);
    return nullptr;
  }

  if (inline_funcs_) {
    for (auto &f : *(runtime.get())) {
      redwoodLLVMContext::mark_inline(&f);
    }
  }

  const bool module_broken = llvm::verifyModule(*runtime.get(), &llvm::errs());
  if (module_broken) {
    RW_ERROR("Broken bitcode={}", bitcode_path_);
    return nullptr;
  }
...
std::unique_ptr<llvm::Module> redwoodLLVMContext::clone_runtime_module() {
  RW_AUTO_PROF
  auto *mod = get_this_thread_runtime_module();

  std::unique_ptr<llvm::Module> cloned;
  {
    RW_PROFILER("clone module");
    cloned = llvm::CloneModule(*mod);
  }

  RW_ASSERT(cloned != nullptr);

  return cloned;
}

std::unique_ptr<llvm::Module> redwoodLLVMContext::module_from_file(
  ...
  if (arch_ == Arch::cuda || arch_ == Arch::amdgpu) {
    auto patch_intrinsic = [&](std::string name, Intrinsic::ID intrin,
      ...
      redwoodLLVMContext::mark_inline(func);
    };
    ...
    auto patch_amdgpu_kernel_dim = [&](std::string name, llvm::Value *lhs) {
      ...
      if (name == "block_dim")
        ...
        RW_ERROR("Unknown patch function name");
      auto func = module->getFunction(name);
...
void redwoodLLVMContext::link_module_with_custom_cuda_library(
  ...
  if (!cuda_library_path.empty()) {
    ...
    if (failed) {
      RW_ERROR("cuda_runtime.bc linking failure.");
    }
  }
}

void redwoodLLVMContext::link_module_with_cuda_libdevice(
    std::unique_ptr<llvm::Module> &module) {
  RW_AUTO_PROF
  RW_ASSERT(arch_ == Arch::cuda);
  ...
  if (failed) {
    RW_ERROR("CUDA libdevice linking failure.");
  }
  ...
  for (auto func_name : libdevice_function_names) {
    ...
    if (!func) {
      RW_INFO("Function {} not found", func_name);
    }
  }
}

void redwoodLLVMContext::link_module_with_amdgpu_libdevice(
    std::unique_ptr<llvm::Module> &module) {
  RW_ASSERT(arch_ == Arch::amdgpu);
#if defined(RW_WITH_AMDGPU)
  ...
  for (auto &libdevice : libdevice_files) {
    ...
    if (failed) {
      RW_ERROR("AMDGPU libdevice linking failure.");
    }
  }
#endif
}

void redwoodLLVMContext::add_struct_module(std::unique_ptr<Module> module,
                                          int tree_id) {
  RW_AUTO_PROF;
  RW_ASSERT(std::this_thread::get_id() == main_thread_id_);
  auto this_thread_data = get_this_thread_data();
  RW_ASSERT(module);
  if (llvm::verifyModule(*module, &llvm::errs())) {
    module->print(llvm::errs(), nullptr);
    RW_ERROR("module broken");
  }
...
template <typename T>
llvm::Value *redwoodLLVMContext::get_constant(DataType dt, T t) {
  auto ctx = get_this_thread_context();
...
template llvm::Value *redwoodLLVMContext::get_constant(DataType dt, int32 t);
template llvm::Value *redwoodLLVMContext::get_constant(DataType dt, int64 t);
template llvm::Value *redwoodLLVMContext::get_constant(DataType dt, uint32 t);
template llvm::Value *redwoodLLVMContext::get_constant(DataType dt, uint64 t);
template llvm::Value *redwoodLLVMContext::get_constant(DataType dt, float32 t);
template llvm::Value *redwoodLLVMContext::get_constant(DataType dt, float64 t);

template <typename T>
llvm::Value *redwoodLLVMContext::get_constant(T t) {
  auto ctx = get_this_thread_context();
  RW_ASSERT(ctx != nullptr);
  using TargetType = T;
...
std::string redwoodLLVMContext::type_name(llvm::Type *type) {
...
std::size_t redwoodLLVMContext::get_type_size(llvm::Type *type) {
  return get_data_layout().getTypeAllocSize(type);
}

std::size_t redwoodLLVMContext::get_struct_element_offset(llvm::StructType *type,
                                                         int idx) {
  return get_data_layout().getStructLayout(type)->getElementOffset(idx);
}

void redwoodLLVMContext::mark_inline(llvm::Function *f) {
  for (auto &B : *f)
    for (auto &I : B) {
...
int redwoodLLVMContext::num_instructions(llvm::Function *func) {
  int counter = 0;
...
void redwoodLLVMContext::print_huge_functions(llvm::Module *module) {
  int total_inst = 0;
  ...
  for (auto &f : *module) {
    ...
    if (c > 100) {
      total_big_inst += c;
      RW_INFO("{}: {} inst.", std::string(f.getName()), c);
    }
    total_inst += c;
  }
  RW_P(total_inst);
...
llvm::DataLayout redwoodLLVMContext::get_data_layout() {
  return data_layout_;
}

void redwoodLLVMContext::insert_nvvm_annotation(llvm::Function *func,
                                               std::string key,
                                               int val) {
  /*******************************************************************
...
void redwoodLLVMContext::mark_function_as_cuda_kernel(llvm::Function *func,
                                                     int block_dim) {
...
void redwoodLLVMContext::mark_function_as_amdgpu_kernel(llvm::Function *func) {
  func->setCallingConv(llvm::CallingConv::AMDGPU_KERNEL);
}

void redwoodLLVMContext::eliminate_unused_functions(
    llvm::Module *module,
...
redwoodLLVMContext::ThreadLocalData *redwoodLLVMContext::get_this_thread_data() {
  std::lock_guard<std::mutex> _(thread_map_mut_);
  auto tid = std::this_thread::get_id();
  if (per_thread_data_.find(tid) == per_thread_data_.end()) {
    std::stringstream ss;
    ss << tid;
    RW_TRACE("Creating thread local data for thread {}", ss.str());
...
llvm::LLVMContext *redwoodLLVMContext::get_this_thread_context() {
  ThreadLocalData *data = get_this_thread_data();
  RW_ASSERT(data->llvm_context)
  return data->llvm_context;
}

llvm::orc::ThreadSafeContext *
redwoodLLVMContext::get_this_thread_thread_safe_context() {
  get_this_thread_context();  // make sure the context is created
  ThreadLocalData *data = get_this_thread_data();
  return data->thread_safe_llvm_context.get();
}

template llvm::Value *redwoodLLVMContext::get_constant(float32 t);
template llvm::Value *redwoodLLVMContext::get_constant(float64 t);

template llvm::Value *redwoodLLVMContext::get_constant(bool t);

template llvm::Value *redwoodLLVMContext::get_constant(int32 t);
template llvm::Value *redwoodLLVMContext::get_constant(uint32 t);

template llvm::Value *redwoodLLVMContext::get_constant(int64 t);
template llvm::Value *redwoodLLVMContext::get_constant(uint64 t);

#ifdef RW_PLATFORM_OSX
template llvm::Value *redwoodLLVMContext::get_constant(unsigned long t);
#endif

auto make_slim_libdevice = [](const std::vector<std::string> &args) {
  RW_ASSERT_INFO(args.size() == 1,
  ...
  os.flush();
  RW_INFO("Slimmed libdevice written to {}", output_fn);
};

void redwoodLLVMContext::init_runtime_module(llvm::Module *runtime_module) {
...
void redwoodLLVMContext::delete_snode_tree(int id) {
  RW_ASSERT(linking_context_data->struct_modules.erase(id));
  for (auto &[thread_id, data] : per_thread_data_) {
    RW_ASSERT(data->struct_modules.erase(id));
  }
}

void redwoodLLVMContext::fetch_this_thread_struct_module() {
  ThreadLocalData *data = get_this_thread_data();
...
llvm::Function *redwoodLLVMContext::get_runtime_function(
    const std::string &name) {
  return get_this_thread_runtime_module()->getFunction(name);
}

llvm::Module *redwoodLLVMContext::get_this_thread_runtime_module() {
  RW_AUTO_PROF;
...
llvm::Function *redwoodLLVMContext::get_struct_function(const std::string &name,
                                                       int tree_id) {
  auto *data = get_this_thread_data();
  return data->struct_modules[tree_id]->getFunction(name);
}

llvm::Type *redwoodLLVMContext::get_runtime_type(const std::string &name) {
  auto ty = llvm::StructType::getTypeByName(
      get_this_thread_runtime_module()->getContext(), ("struct." + name));
  if (!ty) {
    RW_ERROR("LLVMRuntime type {} not found.", name);
  }
  return ty;
}
std::unique_ptr<llvm::Module> redwoodLLVMContext::new_module(
    std::string name,
...
redwoodLLVMContext::ThreadLocalData::ThreadLocalData(
...
LLVMCompiledKernel redwoodLLVMContext::link_compiled_tasks(
...
void redwoodLLVMContext::add_struct_for_func(llvm::Module *module,
                                            int tls_size) {
...
std::string redwoodLLVMContext::get_struct_for_func_name(int tls_size) {
  return "parallel_struct_for_" + std::to_string(tls_size);
}

llvm::DataLayout redwoodLLVMContext::get_data_layout(Arch arch) {
  RW_ASSERT(arch_uses_llvm(arch));
  if (arch_is_cpu(arch)) {
    auto expected_jtmb = llvm::orc::JITTargetMachineBuilder::detectHost();
    if (!expected_jtmb)
      RW_ERROR("LLVM TargetMachineBuilder has failed.");
...
RW_REGISTER_TASK(make_slim_libdevice);
