...
/**
 * Manages an LLVMContext for redwood's usage.
 */
class redwoodLLVMContext {
 private:
 ...
 public:
  ...
  redwoodLLVMContext(const CompileConfig &config, Arch arch);

  virtual ~redwoodLLVMContext();
  ...
  */
  ...
  llvm::Type *get_data_type() {
    return redwoodLLVMContext::get_data_type(redwood::lang::get_data_type<T>());
  }
