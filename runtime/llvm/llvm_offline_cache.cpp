...
#include "redwood/analysis/offline_cache_util.h"
#include "redwood/common/cleanup.h"
#include "redwood/common/version.h"
#include "redwood/ir/transforms.h"
#include "redwood/program/kernel.h"
#include "redwood/runtime/llvm/llvm_context.h"
#include "redwood/util/io.h"
#include "redwood/util/lock.h"
...
static std::string get_llvm_cache_metadata_file_path(const std::string &dir) {
  return redwood::join_path(dir, std::string(kMetadataFilename) + ".tcb");
}

static std::string get_llvm_cache_metadata_json_file_path(
    const std::string &dir) {
  return redwood::join_path(dir, std::string(kMetadataFilename) + ".json");
}
...
struct CacheCleanerUtils<LlvmOfflineCache> {
  ...
  static bool save_metadata(const CacheCleanerConfig &config,
    ...
    write_to_binary_file(
        data, redwood::join_path(config.path, config.metadata_filename));
    return true;
  }
  ...
  static bool save_debugging_metadata(const CacheCleanerConfig &config,
    ...
    ts.write_to_file(
        redwood::join_path(config.path, config.debugging_metadata_filename));
    return true;
  }
...
bool LlvmOfflineCacheFileReader::load_meta_data(
  ...
  if (!redwood::path_exists(tcb_path)) {
  ...
  std::string lock_path =
      redwood::join_path(cache_file_path, kMetadataFileLockName);
...
bool LlvmOfflineCacheFileReader::get_kernel_cache(
  ...
  if (!data.module) {
    std::string filename_prefix = redwood::join_path(path_, key);
  ...
  if (!verified) {
    for (const auto &f : get_possible_llvm_cache_filename_by_key(key)) {
      redwood::remove(redwood::join_path(path_, f));
    }
  }
...
void LlvmOfflineCacheFileWriter::dump(const std::string &path,
  ...
  redwood::create_directories(path);
  ...
  for (auto iter = data_.kernels.begin(); iter != data_.kernels.end(); ++iter) {
    ...
    std::string filename_prefix = redwood::join_path(path, k);
    {
...

  {
    ...
    std::string lock_path = redwood::join_path(path, kMetadataFileLockName);
    if (!lock_with_file(lock_path)) {
      TI_WARN(
