...
//#include "runtime/vulkan/node_dynamic.h"
//#include "runtime/vulkan/node_pointer.h"
namespace redwood{
  namespace runtime{
...
Ptr vulkanRuntime::allocate_aligned(PreallocatedMemoryChunk &memory_chunk,
                                  std::size_t size,
...
/*
#include "runtime/vulkan/module/locked_task.h"
// [ONLY ON DEVICE] CUDA/AMDGPU backend
Ptr vulkanRuntime::allocate_from_reserved_memory(
...
// [ON HOST] CPU backend
// [ON DEVICE] CUDA/AMDGPU backend
void runtime_memory_allocate_aligned(vulkanRuntime *runtime,
                                     std::size_t size,
                                     std::size_t alignment,
                                     uint64_t *result) {
  *result =
...
void runtime_initialize(
  ...
  if (preallocated_size) {
    runtime = (vulkanRuntime *)preallocated_buffer;
    preallocated_buffer +=
        redwood::iroundup(sizeof(vulkanRuntime), redwood_page_size);
  } else {
    runtime =
        (vulkanRuntime *)host_allocator(memory_pool, sizeof(vulkanRuntime), 128);
  }
...
void runtime_initialize_snodes(vulkanRuntime *runtime,
...
void runtime_get_memory_requirements(Ptr result_buffer,
  ...
  if (use_preallocated_buffer) {
    size += redwood::iroundup(int64_t(sizeof(vulkanRuntime)), redwood_page_size);
  }
...
void runtime_NodeAllocator_initialize(vulkanRuntime *runtime,
                                      int snode_id,
...
void runtime_allocate_ambient(vulkanRuntime *runtime,
                              int snode_id,
                              std::size_t size) {
...
void runtime_initialize_memory(vulkanRuntime *runtime,
...
void runtime_initialize_rand_states_cuda(vulkanRuntime *runtime,
...
void runtime_initialize_rand_states_serial(vulkanRuntime *runtime,
                                           int starting_rand_state) {
...
void vulkanRuntime_initialize_thread_pool(vulkanRuntime *runtime,
                                        void *thread_pool,
                                        void *parallel_for) {
